...
class SubTaskDbHelper {
  ...
  static Future<List<Map<dynamic,dynamic>>> getDb (String table,int id) async {
    final db = await  SubTaskDbHelper
    .database();
  ...
  static Future<int> updateRecord(String table,int id,Map<String,Object> data) async{
    final db = await  SubTaskDbHelper
        .database();
    return await db.update(table, data,where: 'subTaskId=?',whereArgs: [id]);
  }

  static Future<int> deleteGroupRecord(String table,int id) async{
    final db = await  SubTaskDbHelper
        .database();
